...
func UpdateAverageAmount() {
	...
	for {
		if aggTradePrice, _ := strconv.ParseFloat(AggTradePrice, 64); !reflect.DeepEqual(aggTradePrice, 0.0) {
			averageSymbol1AmountStr := correction((0.97*free/float64(global.Average))/aggTradePrice, AccountInstance.LotSizeFilter.stepSize)
			global.AverageSymbol1Amount, _ = strconv.ParseFloat(averageSymbol1AmountStr, 64)

			averageSymbol2AmountStr := correction(0.97*free/float64(global.Average), AccountInstance.PriceFilter.tickSize)
